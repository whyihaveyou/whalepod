...
> v0.2：产品定名「鲸群 WhalePod」，标志由 "W" 节点图更换为**四豚环逐**（用户以生成稿 `assets/whalepod-icon-user-ref.png` 定稿，矢量化重绘落地，源文件 `assets/whalepod-icon-final.svg`）。
...
标志图形是**四头鲸豚首尾相逐成环**（90° 旋转对称）：

- 四头 = 多智能体群体（pod），首尾相逐 = 消息传递与接力执行（agent loop / transport 推送）；
